...
# Step 5: Update the repo so ansible doesn't overwrite the key
echo ""
echo "[5/5] Updating repo with new public key..."
HOMELAB_DIR="/root/homelab"
if [ -d "$HOMELAB_DIR" ]; then
  docker cp "$GITLAB_CONTAINER:/tmp/license_key.pub" "$HOMELAB_DIR/docker/gitlab/license_encryption_key.pub"
  cd "$HOMELAB_DIR"
  git add docker/gitlab/license_encryption_key.pub
  git commit -m "chore: update license encryption public key"
  git push origin main
  echo "  Public key committed to repo."
else
  echo "  WARNING: $HOMELAB_DIR not found. You must manually copy the public key"
  echo "  to docker/gitlab/license_encryption_key.pub in the homelab repo,"
  echo "  otherwise the next pipeline deploy will overwrite it and break the license."
  echo ""
  echo "  To copy it out: docker cp gitlab:/tmp/license_key.pub ./license_encryption_key.pub"
fi
echo ""
