...
## 23:41 — CODEOWNERS + Temporal Progress

### MR !252: CODEOWNERS Ansible Wildcard
- Replaces 4 specific ansible rules with single `/ansible/ @root`
- Awaiting Noah's approval

### Temporal Worker Active
- Initially appeared stuck (0 tokens in sessions_list) but was actually running
- sessions_list can show stale token counts — use sessions_history to verify
- Worker is doing Greenhouse application, hit email verification step, using JIT Gmail

### Today's MR Summary
- !249: Chromium resume volume mount (needs merge)
- !250: Chromium static IP 10.3.32.9 (needs merge)  
- !251: Chromium daily restart cron (merged, self-merged — exposed CODEOWNERS gap)
- !252: CODEOWNERS ansible wildcard fix (needs Noah approval)

## 23:51 — Gmail OAuth Setup

### OAuth Credentials (in Vault once Noah writes them)
- Client ID: 686435669041-ci8f7mjncuqv94limht8ngbmlcij2cmj.apps.googleusercontent.com
- Vault path target: `homelab/data/agents/gmail_oauth` or `homelab/data/moltbot/gmail_oauth`
- Scope: `gmail.readonly` (for fetching verification codes)
- Redirect URI: `http://localhost` (OOB deprecated by Google)

### Vault Write Access
- moltbot-ops AppRole policy is READ-ONLY for both `homelab/data/agents/*` and `homelab/data/moltbot/*`
- Cannot write secrets — Noah needs to write them or policy needs updating

### Temporal Application Status
- Form fully filled, resume uploaded, all fields complete
- BLOCKED on Greenhouse email verification code
- Noah hasn't provided the code yet — may have expired by now
- Gmail access for workers is the systemic fix

### Refresh Token Flow
- Sent Noah the OAuth consent URL
- Waiting for auth code to exchange for refresh token
- Once we have refresh token, workers can fetch verification emails autonomously

## 00:13 (Feb 13) — Gmail OAuth Complete

### Gmail API Access Working
- OAuth creds in Vault at `homelab/data/docker/gmail` (client_id, client_secret, refresh_token, email)
- Gmail API enabled in Google Cloud project 686435669041
- Refresh token tested and working — can fetch messages via Gmail REST API
- Scope: `gmail.readonly`
- Workers can now autonomously fetch verification codes for job applications

### JIT Vault Paths Schema (LEARNED)
- `vault_paths` is an array of objects: `[{"path": "...", "capabilities": ["read","create","update"]}]`
- NOT a string array, NOT a single string
- Added `jit_vault_write` and `vault_write` helpers to jit-lib.sh

### vault_read from jit-lib
- Uses AppRole token which only reads `homelab/data/agents/*` and `homelab/data/moltbot/*`
- For other paths (like `homelab/data/docker/*`), need JIT vault token
- `jit_vault` helper handles this

### Temporal Application
- Still needs Greenhouse verification code — but code likely expired by now
- Will need to retry with Gmail access now working

### Pending for Noah
- Enable Gmail API ✅ DONE
- Merge MR !249 (resume volume mount)
- Merge MR !250 (chromium static IP)
- Approve MR !252 (CODEOWNERS ansible wildcard)
