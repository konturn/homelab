...
## 10:00-18:45 — JIT Service Major Expansion + Testing

### JIT Webhook Fix (Root Cause Found)
- **Root cause:** nginx (external network 10.2.x.x) couldn't reach JIT service (internal 10.3.x.x) — different Docker networks
- **Fix:** MR !168 — added `jit-bridge` Docker network between nginx and jit-approval-svc
- **SSL cert** was also missing `jit-webhook.nkontur.com` but auto-renewed at 08:02 UTC
- **Webhook "clearing" mystery** was a red herring — webhook was set fine, but nginx timeouts made it look broken. The getWebhookInfo response parsing was also wrong on my part
- **Full e2e Telegram approval flow confirmed working** — all 11 services tested

### JIT Dynamic Backends Built
- **MR !169** — Dynamic GitLab project access tokens (merged, working)
  - Uses root PAT to mint scoped project access tokens
  - Initial failure: seeded a project bot token instead of real user PAT — project bots can't create tokens
  - Fixed by seeding root's PAT (`glpat-FgXQU1co...`)
- **MR !170** — Scoped GitLab tokens — callers specify `scopes: ["read_api"]` etc.
  - Tested: read_api works for reads, blocks writes with `insufficient_scope` ✅
- **MR !172** — Added Tailscale (T2 dynamic), Prowlarr (T1 static), MQTT (T1 static), Pihole (T2 static)
- **MR !173** — Dynamic Vault backend with inline scoped policies 🔥
  ...
  - Tested end-to-end: read allowed paths ✅, denied unrequested paths ✅, denied writes when only read requested ✅
- **MR !174** — Merged T0 into T1. Tiers now represent approval trust, not backend type

### Vault Audit Logging
- **MR !171** — Enabled Vault file audit + Promtail shipping to Loki
- Confirmed working — can see all Vault operations in Loki with HMAC'd tokens

### Other MRs Created
- **!160** — Mosquitto image digest typo fix
- **!161** — Jackett update + image pinning
- **!162** — no-new-privileges on gitlab, pihole, zigbee2mqtt, wordpress_db
- **!163** — Separate root/app DB passwords
- **!164** — Document HA CAP_NET_ADMIN requirement
- **!165** — Backup monitoring alerts
- **!166** — Closed (Watchtower doesn't exist in stack)
- **!167** — JIT webhook self-registration on startup

### Operational
- Removed `open-mrs.json` tracking — query GitLab API directly instead
...
### Key Lessons
- Docker cross-network communication is the most common "it's not working" issue — always check network topology first
- Project bot users (from project access tokens) have limited permissions — can't create other tokens
...
## 21:00 — Evening Recap
- MRs !175 (SSH certs), !176 (gmail JIT + moltbot vault read), !177 (vault-read audit sudo) all merged
- !172 (tailscale/prowlarr/mqtt/pihole) rebased with 6 conflict resolution, pipeline green, awaiting review
- Hotfixed vault-read policy (sys/audit needs sudo) — unblocked all MR pipelines
- Main branch protection enabled — no more direct pushes, MRs only
- Gmail secret seeded in Vault via CI/CD VAULT_TOKEN + GitLab API
- Pipeline 2064 failure (sshd→ssh) auto-fixed by cron-spawned sub-agent, pipeline 2067 green
- Updated TOOLS.md tier reference (T0 merged into T1 per !174)
- Discussed security: env injection vs vault pull vs JIT, moving T1 static to direct vault reads
- Meditation: "The Gardener's Paradox"
