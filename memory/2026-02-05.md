...
## ~17:54-18:00 - Final MR Updates

### MR !91 — SSH WAN Access Added
- Noah asked for out-of-band SSH access from internet when Tailscale is down
- Added `-A INPUT -i {{ inet_interface_name }} -p tcp -m tcp --dport 22 -j ACCEPT`
- SSH now allowed from: mgmt VLAN + Tailscale + WAN (key-only auth)
- MR !91 now has 4 changes: SSH restriction (with WAN), docker socket removal, nextcloud_db isolation, WordPress healthcheck, Vault TLS

### GitLab License Discussion
- Noah said "we'll discuss that later" re: MR !102
- License is generated and ready, just needs his review/merge

### Long-term Security Architecture (Noah's vision — SAVE THIS)
- **Declarative access only**: I write code/MRs, pipeline deploys
- **Observability access**: Grafana, InfluxDB, Loki, Portainer API
- **No runtime access**: For direct diagnosis, I compose context+commands, Noah runs via Claude Code on router
- **Restricted self-merge**: Safe paths = self-merge, sensitive paths = Noah approval
- This is the direction we're heading once GitLab Ultimate features are available (CODEOWNERS, required approvals)

### Session Summary — Massive Infrastructure Day
Today's MRs created/merged:
- !90 Trivial fixes — MERGED (self)
- !91 Security hardening — Updated, awaiting review
- !92 Reliability/resource limits — MERGED (Noah)
- !93 CI deploy always on main — MERGED (self)
- !95 Restic role vendored — MERGED (self)
- !96 Deploy fixes (Debian/NFS) — MERGED (self)
- !97 Loki resilience — MERGED (Noah)
- !98 msmtp email notifications — Awaiting review
- !99 Bullseye Docker skip — MERGED (self)
- !100 Bullseye Docker proper — MERGED (self)
- !101 Deluge host check — MERGED (self)
- !102 GitLab Ultimate license — Awaiting review

Plus: image-update skill+cron built, watchtower removed, model upgraded to Opus 4.6

## 18:28 - Vault Secrets Migration

Major infrastructure task: migrated all homelab secrets from GitLab CI/CD variables to HashiCorp Vault.

### Vault Setup
- Initialized Vault at 10.3.32.6:8200 (5 unseal keys, 3 threshold)
- Root token: PURGED (was in memory/vault-init-keys.json, now deleted)
- Enabled KV v2 engine at `homelab/` mount
- Populated 41 secret groups (70+ individual values)

### Vault Secret Organization
- `homelab/infrastructure/*` — LUKS, IPMI, SNMP, router, Tailscale, pihole, etc.
- `homelab/networking/*` — Cloudflare, Namesilo, Wireguard, Mullvad
- `homelab/docker/*` — Per-service (influxdb, grafana, plex, moltbot, etc.)
- `homelab/backup/*` — Backblaze, Restic, Borg
- `homelab/mqtt/*`, `homelab/cameras/*`, `homelab/email/*`
- `homelab/api-keys/*`, `homelab/moltbot/*`, `homelab/gitlab/*`

### MR !102
- Replaced 50 `lookup('env', ...)` calls across 12 files
- Created centralized `ansible/vault_vars.yml`
- Added `community.hashi_vault` collection + `hvac` package
- Pipeline passed ✅
- NOT self-merging — major infrastructure change

### Important Notes
- Vault auto-seals on container restart — needs manual unseal
- Root token purged from disk; AppRole should be set up for moltbot access
- Vault runs HTTP internally (TLS not yet functional on listener)
- Existing GitLab CI variables preserved for parallel operation
-e 
## 19:30 - GitLab Performance + Vault Auto-Unseal MRs

Created two MRs per Noah's request:

**MR !111** - GitLab Performance Tuning (feat/gitlab-performance-tuning)
- gitlab.rb: Puma 8→4 workers, threads 1-4→4-8, Sidekiq 10→5
- Disabled monitoring (prometheus, exporters, grafana), unused features (KAS, Pages, Mattermost, etc.)
- PostgreSQL: shared_buffers 2G→1G, work_mem 64M→32M, slow query log >500ms
- Redis: 512MB maxmemory with allkeys-lru
- jemalloc MALLOC_CONF env var on container
- Runner concurrent 20→2
- Nginx: added gitlab.lab.nkontur.com server block with HTTP/2 + proxy_buffers 8 32k
- Pipeline passed ✅

**MR !108** - Vault Auto-Unseal via CI/CD (updated existing branch)
- Created VAULT_UNSEAL_KEYS protected CI/CD variable with first 3 hex unseal keys
- Added Ansible task to write keys from CI env to /persistent_data/application/vault/unseal/unseal-keys
- Updated README: removed manual key placement instructions
- Pipeline passed ✅

Notified Noah via Telegram. Not self-merging either (architectural changes).

## ~18:00-20:15 - Security Architecture, Vault, GitLab Performance, Massive MR Sprint

### Security Architecture (IMPORTANT — Long-term design)
- Noah's vision: declarative access (code/MRs) only, no imperative (runtime) access
- Observability access (Grafana, InfluxDB, Loki, Portainer API)
- For runtime diagnosis: I compose context+commands, Noah runs via Claude Code on router
- CODEOWNERS implemented! MR !109 (draft) — protects networking, iptables, SSH, CI, compose, moltbot config, vault, CODEOWNERS itself
- GitLab Ultimate license working — MR !102 merged, CODEOWNERS enforcement active
- Self-merge test passed: MR !110 (dead code cleanup) merged without issues on unprotected paths

### GitLab Performance Tuning (MR !111 — CRASHED GITLAB)
- Puma 8→4 workers, threads 1-4→4-8, Sidekiq 10→5
- jemalloc MALLOC_CONF tuning
- Disabled monitoring (prometheus, exporters)
- Runner concurrent 20→2
- PostgreSQL shared_buffers 2G→1G, work_mem 32MB, slow query logging
- **CRASHED** due to deprecated config keys in 18.8.3:
  - grafana['enable'] — removed in GitLab 16.3 ✅ confirmed
  - dependency_proxy['enable'] — never a valid top-level key
  - terraform_state['enable'] — never a valid top-level key
  - packages['enable'] — never a valid top-level key
- redis['maxmemory'] and redis['maxmemory_policy'] ARE valid (Claude Code on router was wrong, official docs confirm)
- Fix in MR !113

### LESSON: Claude Code on Router vs Me
- Router Claude said redis maxmemory keys were invalid — WRONG per official docs
- I originally hallucinated dependency_proxy/terraform_state/packages as "removed" — they were never top-level keys to begin with
- Always verify against official docs, don't trust either Claude instance blindly

### Vault Progress
- MR !108 merged — auto-unseal via CI/CD variable (VAULT_UNSEAL_KEYS)
- MR !112 — vault unseal task restricted to router-only (was running on all hosts)
- Vault Terraform config — MR !114 ready:
  - JWT auth for CI runners (project 4, main branch, 5-min TTL)
  - AppRole auth for moltbot (1h TTL, scoped to operational secrets only)
  - ci-deploy policy: full homelab/* read
  - moltbot-ops policy: only plex/radarr/sonarr/grafana/influxdb/mqtt/cameras etc.
  - terraform/vault/ directory with 6 files

### Two-tier Vault Access Design
- **ci-deploy**: runners need everything to deploy full stack
- **moltbot-ops**: I only need service API keys, NOT infrastructure secrets
  - No LUKS, SSH keys, Cloudflare, DB passwords, Wireguard
  - Blast radius if compromised: can read Plex API key, not deploy credentials

### Other MRs
- MR !104 — no_log for secrets in CI diffs (YAML fix pushed)
- MR !105 — amcrest2mqtt TLS retained (patched upstream TLS+password bug, correct Alpine CA path)
- MR !107 — Traefik migration design doc (recommends Caddy over Traefik)
- MR !110 — dead code cleanup MERGED (self-merge test passed)
- MR !113 — GitLab deprecated config fix (redis settings restored)

### Issue #29 — dind Service Validation
- Noah's idea: spin up services in docker:dind during CI to validate config changes
- Would have caught the GitLab crash before merge
- Phase 1: GitLab, Phase 2: other critical services, Phase 3: full stack

### Terraform for Vault Config
- Recommended Terraform over Ansible for Vault config management
- Declarative state tracking vs imperative playbooks
- Vault provider manages auth backends, policies, roles, secret engines

### Noah Trip
- Leaving Feb 7 for Arizona to visit Avery, returning Feb 16 (9 days)
- Will have laptop for occasional merges
- Standing rule: DO NOT self-merge anything touching networking/remote access while he's away
- Keep overnight routine going, ping on Telegram only for urgent stuff

### Creds Purge (TODO after Vault migration complete)
- Noah wants ALL credentials purged from memory files AND git history
- git filter-repo to rewrite history
- Standing rule going forward: NEVER write credentials to memory files
- Use CI variables or Vault only

### Sub-agent Queue Issues
- Heavy day caused rate limiting — sub-agents queued 15-25 min at 0 tokens
- archiveAfterMinutes: 30 killed one sub-agent that was still queued
- Consider bumping archiveAfterMinutes on heavy days
- Sub-agents losing task context when queued too long (sessions_send to re-inject)

## 20:15-22:22 — Vault DNS Debug, Security Architecture, Bootstrap, SMS

### Vault DNS Resolution Saga
- vault:configure CI job kept failing during terraform apply
- **First issue:** VAULT_ADDR CI variable was `http://` not `https://` — fixed
- **Second issue:** vault.lab.nkontur.com resolved to 10.3.32.1 (router) not 10.3.32.6 (vault)
- Spent time on wrong theories (dnsmasq precedence, custom.list vs address= directives)
- Noah kept pushing back ("I don't buy that") — was right every time
- **Actual root cause:** Pi-hole v6 ignores `custom.list` file. DNS records must be in `pihole.toml` via `dns.hosts`. The vault entry in custom.list was never loaded.
- Noah added vault manually to pihole.toml, vault:configure passed — 6 Terraform resources created

### Pi-hole v5→v6 Migration Gap (Issue #30, MR !120)
- custom.list is DEAD in Pi-hole v6 — all entries ignored
- Other services (snapserver, mqtt) worked because they were already in pihole.toml (probably migrated during v6 upgrade)
- Vault was added post-upgrade, only went to custom.list
- Created MR !120 to migrate all 17 entries to Ansible-managed pihole.toml via `pihole-FTL --config dns.hosts`

### Vault JWT Auth (MR !116)
- vault-admin policy + role: write access, main branch only, for terraform apply
- vault-read policy + role: read-only, ANY branch, for MR terraform plan
- vault:configure uses vault-admin, vault:validate uses vault-read
- Least-privilege scoping — MR pipelines can't write to Vault

### Git Worktree Fix (IMPORTANT)
- **Root cause of all MR contamination tonight:** All sub-agents share `/home/node/.openclaw/workspace/homelab`
- When multiple sub-agents checkout different branches concurrently, dirty files leak between them
- **Fix:** Updated gitlab-mr and gitlab-mr-create skills to use `git worktree` for branch isolation
- Each sub-agent gets `/tmp/homelab-$BRANCH` as isolated working directory
- Shared object store, isolated working trees — no more cross-contamination

### Bootstrap & Disaster Recovery (MR !121)
- `scripts/bootstrap.sh` — bare metal to CI in one script
- `router:bootstrap` — manual CI stage for Vault, Pi-hole, switch config
- Restic restore MUST happen in bootstrap.sh BEFORE GitLab starts (GitLab data is in the backup)
- Router network config can wait for first CI pipeline run of router.yml
- 5 things needed offsite for full recovery: B2 account ID, B2 key, restic password, vault unseal keys, bootstrap.sh

### Security Discussion
- Compose file should stay in Noah's approval path — volume mounts are too powerful
- I could add volume mounts to any service I control to exfiltrate data via Loki logs
- CODEOWNERS on compose is the honest answer, not split files

### New Standing Directives
- **Nightly red team exercise** — assume compromised, find vulns, document findings
- **9 AM daily digest** — consolidated Telegram message with overnight work (cron job set up)
- **Don't comment removed code** — git history is for that

### Radarr/Sonarr Performance (MR !118)
- Applied API changes: disabled searchOnAdd on 2/4 import lists, triggered housekeeping on both
- Disabled analytics on both
- IMDb import lists (IDs 2 and 4) have broken validation — can't update via API
- Sonarr housekeeping doesn't prune history (308K records persist) — would need direct SQLite

### Images Runner Removed (MR !114)
- Dead runner config cleaned up, self-merged
- Deleted orphaned IMAGES_GITLAB_RUNNER_TOKEN CI variable

### VoIP.ms Research (for SMS capability)
- Best option for cheap SMS: VoIP.ms (~$0.85/mo + $0.0085/msg)
...
- Alternative: Telnyx (more modern API, slightly more expensive)

### MR Status End of Night
- !113 (GitLab deprecated config fix) — awaiting merge
- !116 (Vault JWT bootstrap) — clean, awaiting merge
- !118 (Radarr/Sonarr memory) — awaiting merge
- !120 (Pi-hole DNS migration) — clean, awaiting merge
- !121 (Bootstrap + DR) — sub-agent still updating with restic restore
